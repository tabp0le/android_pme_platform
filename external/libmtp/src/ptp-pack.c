...
ptp_pack_string(PTPParams *params, char *string, unsigned char* data, uint16_t offset, uint8_t *len)
{
	...
	memset(ucs2strp, 0, sizeof(ucs2str));  
#ifdef HAVE_ICONV
	{ 
		size_t nconv;
		size_t convmax = PTP_MAXSTRLEN * 2; 
		char *stringp = string;
	...
	htod16a(&data[offset+packedlen*2+1], 0x0000);  
...
#if 0
	char *capture_date="20020101T010101"; 
#endif
...
#define PTP_ece_Prop_Subtype	8	
#define PTP_ece_Prop_Val_Data	0xc	
#define PTP_ece_Prop_Desc_Type	0xc	
#define PTP_ece_Prop_Desc_Count	0x10	
#define PTP_ece_Prop_Desc_Data	0x14	
...
#undef XX
				default:
					...
					for (j=0;j<(size-PTP_ece_Prop_Desc_Data)/4;j++, data+=4) 
...
ptp_unpack_Nikon_EC (PTPParams *params, unsigned char* data, unsigned int len, PTPContainer **ec, int *cnt)
{
	...
	*cnt = dtoh16a(&data[PTP_nikon_ec_Length]);
	if (*cnt > (len-PTP_nikon_ec_Code)/PTP_nikon_ec_Size) 
		return;
...
#define ptp_canon_dir_previd	0x10	
#define ptp_canon_dir_nextid	0x14	
#define ptp_canon_dir_nextchild	0x18	
#define ptp_canon_dir_storageid	0x1c	
