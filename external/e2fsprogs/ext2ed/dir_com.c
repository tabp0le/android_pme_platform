...

{
	...
	info_ptr->inode_offset=device_offset;				
	...
	info_ptr->level=0;						
	info_ptr->display=HEX;						

	info_ptr->dir_entry_num=0;info_ptr->dir_entries_count=0;	
	info_ptr->dir_entry_offset=0;

	

	info_ptr->dir_entries_count=count_dir_entries (); 		
...

{
	...
	while (info.file_offset < info.file_length) {				

		*status=return_code=action (&info);				
		...
		if (info.dir_entry_offset >= file_system_info.block_size) {	
...

{
	...
	if (status==FOUND) {					
		file_info=info;					
		dispatch ("remember internal_variable");	

		dispatch ("followinode");			

		if (S_ISLNK (type_data.u.t_ext2_inode.i_mode)) {

			if (type_data.u.t_ext2_inode.i_size > 60) {	
			...
			dispatch ("recall internal_variable");	
			dispatch ("dir");			

			sprintf (temp,"cd %s",temp2);		
			dispatch (temp);			
...

{
	...
	inode_offset=inode_num_to_inode_offset (dir_entry_ptr->inode);			
	sprintf (buffer,"setoffset %ld",inode_offset);dispatch (buffer);		
