  ...
  for (k = 0; k < (std::max)(nct,nrt); ++k)
  {
    ...
    for (j = k+1; j < n; ++j)
    {
      if ((k < nct) && (m_sigma[k] != 0.0))
      {
        ...
        Scalar t = matA.col(k).end(m-k).eigen2_dot(matA.col(j).end(m-k)); 
        t = -t/matA(k,k);
  ...
  if (wantu)
  {
    ...
    for (k = nct-1; k >= 0; k--)
    {
      if (m_sigma[k] != 0.0)
      {
        for (j = k+1; j < nu; ++j)
        {
          Scalar t = m_matU.col(k).end(m-k).eigen2_dot(m_matU.col(j).end(m-k)); 
          t = -t/m_matU(k,k);
  ...
  if (wantv)
  {
    for (k = n-1; k >= 0; k--)
    {
      if ((k < nrt) & (e[k] != 0.0))
      {
        for (j = k+1; j < nu; ++j)
        {
          Scalar t = m_matV.col(k).end(n-k-1).eigen2_dot(m_matV.col(j).end(n-k-1)); 
          t = -t/m_matV(k+1,k);
...
SVD<MatrixType>& SVD<MatrixType>::sort()
{
  ...
  for (int i=0; i<n; ++i)
  {
    ...
    if (k != i)
    {
      m_sigma.coeffRef(k) = m_sigma.coeff(i);  
      m_sigma.coeffRef(i) = p;                 
...
{
  ei_assert(m_matU.rows() == m_matV.rows() && "Polar decomposition is only for square matrices");
  Scalar x = (m_matU * m_matV.adjoint()).determinant(); 
