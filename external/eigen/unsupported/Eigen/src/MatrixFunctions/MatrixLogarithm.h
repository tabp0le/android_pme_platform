...
private:
  ...
  static const int maxPadeDegree = std::numeric_limits<RealScalar>::digits<= 24?  5:  
...
void MatrixLogarithmAtomic<MatrixType>::computeBig(const MatrixType& A, MatrixType& result)
{
  ...
  const RealScalar maxNormForPade = maxPadeDegree<= 5? 5.3149729967117310e-1:                     
...
{
  const float maxNormForPade[] = { 2.5111573934555054e-1  , 4.0535837411880493e-1,
...
{
  const double maxNormForPade[] = { 1.6206284795015624e-2  , 5.3873532631381171e-2,
...
{
#if   LDBL_MANT_DIG == 53         
  const long double maxNormForPade[] = { 1.6206284795015624e-2L  , 5.3873532631381171e-2L,
            1.1352802267628681e-1L, 1.8662860613541288e-1L, 2.642960831111435e-1L };
#elif LDBL_MANT_DIG <= 64         
  const long double maxNormForPade[] = { 5.48256690357782863103e-3L , 2.34559162387971167321e-2L,
            5.84603923897347449857e-2L, 1.08486423756725170223e-1L, 1.68385767881294446649e-1L,
            2.32777776523703892094e-1L };
#elif LDBL_MANT_DIG <= 106        
  const long double maxNormForPade[] = { 8.58970550342939562202529664318890e-5L ,
...
#else                             
  const long double maxNormForPade[] = { 4.7419931187193005048501568167858103e-5L ,
