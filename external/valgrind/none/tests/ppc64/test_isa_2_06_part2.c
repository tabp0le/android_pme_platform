...
#define VECTOR_ADDR(_v) ((void *)&_v)
            ...
            if (repeat) {
               memcpy(VECTOR_ADDR(vec_out), inB, 8);  
               memcpy(VECTOR_ADDR(vec_inB), &spec_fargs[fp_idx[0]], 8);  
               frbp = (unsigned long long *)&spec_fargs[fp_idx[0]];
            } else {
               frbp = (unsigned long long *)&spec_fargs[aTest.frb_idx];
               memcpy(VECTOR_ADDR(vec_inB), inB, 8);  
               memcpy(VECTOR_ADDR(vec_out), &spec_fargs[fp_idx[0]], 8);  
            }
