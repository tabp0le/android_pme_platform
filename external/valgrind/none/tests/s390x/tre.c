...
int main()
{
   ...
   memset(buff, 'a', 1);
   run_test((char *)&touppercase, &buff, 1, 'a');   
   run_test((char *)&touppercase, &buff, 1, 'b');

   memcpy(buff, "abcdefgh", 8);
   run_test((char *)&touppercase, &buff, 3, 'a');   
   run_test((char *)&touppercase, &buff, 3, 'f');   
   run_test((char *)&touppercase, &buff, 8, 'l');   

   memcpy(buff, "ABCDEFGH", 8);
   run_test((char *)&tolowercase, &buff, 3, 'A');   
   run_test((char *)&tolowercase, &buff, 3, 'C');   
   run_test((char *)&tolowercase, &buff, 8, 0x0);   
