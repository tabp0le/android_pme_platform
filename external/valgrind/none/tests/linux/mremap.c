...
int main()
{
	...
	p = mkmap(1024*1024);
	np = mremap(p, 1024*1024, 256*1024, 0);		
	if (np == (char *)-1)
	...
	p = mkmap(1024*1024);
	np = mremap(p, 1024*1024, 256*1024, MREMAP_MAYMOVE);		
	if (np == (char *)-1)
	...
	p = mkmap(1024*1024);
	np = mremap(p, 1024*1024, 2048*1024, 0); 
	if (np == (char *)-1)
	...
	p = mkmap(1024*1024);
	np = mremap(p, 1024*1024, 2048*1024, MREMAP_MAYMOVE); 
	if (np == (char *)-1)
	...
	np = mremap(p, 512*1024, 1024*1024, 0); 
	if (np == (char *)-1)
	...
	np = mremap(p, 512*1024, 1024*1024, MREMAP_MAYMOVE); 
	if (np == (char *)-1)
