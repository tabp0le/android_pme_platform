...
enum test_flags {
    ...
    PPC_LD_ARGS    = 0x00000008,  
    PPC_LDX_ARGS   = 0x00000009,  
    PPC_ST_ARGS    = 0x0000000A,  
    PPC_STX_ARGS   = 0x0000000B,  
    PPC_ONE_IMM    = 0x0000000C,  
...
static void build_iargs_table (void)
{
   ...
   if (arg_list_size == 1) {                   
      ...
      for (tmp=0; ; tmp = 999999*tmp + 999999) {  
         if (tmp >= 0x100000000ULL)
            tmp = 0xFFFFFFFF;
         iargs[i++] = (HWord_t)tmp;
         AB_DPRINTF("val %08x\n", (HWord_t)tmp);
                           ...
                           unused uint32_t test_flags)
{
   ...
   base  = (HWord_t)data;
   size_t offs = 100 * sizeof(unsigned long);    
