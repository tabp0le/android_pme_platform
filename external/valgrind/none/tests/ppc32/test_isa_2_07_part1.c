...
enum test_flags {
    ...
    PPC_STQ_ARGS   = 0x0000000C,  
    PPC_LDQ_ARGS   = 0x0000000D,  
    PPC_STQX_ARGS  = 0x0000000E,  
    PPC_LDQX_ARGS  = 0x0000000F,  
...
static unsigned long long vbcd_args[] __attribute__ ((aligned (16))) = {
   0x8045090189321003ULL, 
   0x001122334556677dULL,
   0x0000107600000001ULL, 
                                        ...
                                        unused uint32_t test_flags)
{
   ...
   mem_priv = memalign16(2 * sizeof(Word_t));  
                       ...
                       char *filter)
{
   ...
   for (i=0; all_tests[i].name != NULL; i++) {
      ...
      switch (family) {
      case PPC_INTEGER:
         mem_resv = memalign16(2 * sizeof(HWord_t));  
