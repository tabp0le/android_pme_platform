...
PRE(sys_sigprocmask)
{
   ...
   SET_STATUS_from_SysRes(
      VG_(do_sys_sigprocmask) ( tid, ARG1 , 
                                set ? &bigger_set    : NULL,
...
PRE(sys_ioctl)
{
   *flags |= SfMayBlock;
   ...
   case VKI_CDROMPLAYMSF:
      ...
      break;
   case VKI_CDROM_DRIVE_STATUS: 
   case VKI_CDROM_CLEAR_OPTIONS: 
      break;
   case VKI_CDROM_GET_CAPABILITY: 
      break;
   ...
   if (KernelVariantiS(KernelVariant_android_gpu_sgx5xx,
      ...
      if (ARG2 >= 0xC01C6700 && ARG2 <= 0xC01C67FF && ARG3 >= 0x1000) {
         if (1) {
            ...
            UInt* word = (UInt*)ARG3;
            if (word && word[2] && word[3] < 0x200)
               POST_MEM_WRITE(word[2], word[3]); 
            if (word && word[4] && word[5] < 0x200)
               POST_MEM_WRITE(word[4], word[5]); 
         }
