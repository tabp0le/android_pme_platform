...
typedef
   struct {
      ...
      ULong stats_n_read_unz_bytes; 
      ULong stats_n_read_z_bytes;   
   }
   ConnState;
...
static void write_UInt_le ( UChar* dst, UInt n )
{
   Int i;
   for (i = 0; i <= 3; i++) {
      dst[i] = (UChar)(n & 0xFF);
...
static void write_ULong_le ( UChar* dst, ULong n )
{
   Int i;
   for (i = 0; i <= 7; i++) {
      dst[i] = (UChar)(n & 0xFF);
...
static UInt calc_gnu_debuglink_crc32(Bool* ok, int fd, ULong size)
{
...
assert(ret != -1);
      ...
      for (i = 0; i < M_CONNECTIONS; i++) {
         ...
         tmp_pollfd[j].events  = POLLIN ;
         tmp_pollfd[j].revents = 0;
         tmp_pollfd_to_conn_state[j] = i;
         j++;
      }

      res = poll(tmp_pollfd, j, 20  );
      if (res < 0) {
