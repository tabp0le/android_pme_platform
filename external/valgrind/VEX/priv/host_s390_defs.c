...
static Int gpr_index[16];  
static Int fpr_index[16];  
...
s390_hreg_gpr(UInt regno)
{
   ...
   vassert(ix >= 0);
   return mkHReg(False, HRcInt64, regno, ix);
}
...
s390_hreg_fpr(UInt regno)
{
   ...
   vassert(ix >= 0);
   return mkHReg(False, HRcFlt64, regno, ix);
}
...
s390_insn_get_reg_usage(HRegUsage *u, const s390_insn *insn)
{
   ...
   case S390_INSN_ALU:
      ...
      addHRegUse(u, HRmRead,  insn->variant.alu.dst);  
      s390_opnd_RMI_get_reg_usage(u, insn->variant.alu.op2);
      break;

   case S390_INSN_SMUL:
   case S390_INSN_UMUL:
      addHRegUse(u, HRmRead,  insn->variant.mul.dst_lo);  
   ...
   case S390_INSN_DIVS:
      ...
      addHRegUse(u, HRmWrite, insn->variant.divs.op1); 
      addHRegUse(u, HRmWrite, insn->variant.divs.rem); 
   ...
   case S390_INSN_BFP_TRIOP:
      ...
      addHRegUse(u, HRmRead,  insn->variant.bfp_triop.dst);  
      addHRegUse(u, HRmRead,  insn->variant.bfp_triop.op2);  
      addHRegUse(u, HRmRead,  insn->variant.bfp_triop.op3);  
      break;

   case S390_INSN_BFP_BINOP:
      addHRegUse(u, HRmWrite, insn->variant.bfp_binop.dst_hi);
      addHRegUse(u, HRmRead,  insn->variant.bfp_binop.dst_hi);  
      addHRegUse(u, HRmRead,  insn->variant.bfp_binop.op2_hi);  
      if (insn->size == 16) {
         addHRegUse(u, HRmWrite, insn->variant.bfp_binop.dst_lo);
         addHRegUse(u, HRmRead,  insn->variant.bfp_binop.dst_lo);  
         addHRegUse(u, HRmRead,  insn->variant.bfp_binop.op2_lo);  
      }
      break;

   case S390_INSN_BFP_UNOP:
      addHRegUse(u, HRmWrite, insn->variant.bfp_unop.dst_hi);
      addHRegUse(u, HRmRead,  insn->variant.bfp_unop.op_hi);  
      if (insn->size == 16) {
         addHRegUse(u, HRmWrite, insn->variant.bfp_unop.dst_lo);
         addHRegUse(u, HRmRead,  insn->variant.bfp_unop.op_lo);  
      }
      break;

   case S390_INSN_BFP_COMPARE:
      addHRegUse(u, HRmWrite, insn->variant.bfp_compare.dst);
      addHRegUse(u, HRmRead,  insn->variant.bfp_compare.op1_hi);  
      addHRegUse(u, HRmRead,  insn->variant.bfp_compare.op2_hi);  
      if (insn->size == 16) {
         addHRegUse(u, HRmRead,  insn->variant.bfp_compare.op1_lo);  
         addHRegUse(u, HRmRead,  insn->variant.bfp_compare.op2_lo);  
      }
      break;
   ...
   case S390_INSN_DFP_BINOP: {
      ...
      addHRegUse(u, HRmRead,  dfp_binop->op2_hi);  
      addHRegUse(u, HRmRead,  dfp_binop->op3_hi);  
      if (insn->size == 16) {
         addHRegUse(u, HRmWrite, dfp_binop->dst_lo);
         addHRegUse(u, HRmRead,  dfp_binop->op2_lo);  
         addHRegUse(u, HRmRead,  dfp_binop->op3_lo);  
      }
      break;
   }
   ...
   case S390_INSN_DFP_UNOP:
      ...
      addHRegUse(u, HRmRead,  insn->variant.dfp_unop.op_hi);  
      if (insn->size == 16) {
         addHRegUse(u, HRmWrite, insn->variant.dfp_unop.dst_lo);
         addHRegUse(u, HRmRead,  insn->variant.dfp_unop.op_lo);  
      }
      break;
   ...
   case S390_INSN_DFP_COMPARE:
      ...
      addHRegUse(u, HRmRead,  insn->variant.dfp_compare.op1_hi);  
      addHRegUse(u, HRmRead,  insn->variant.dfp_compare.op2_hi);  
      if (insn->size == 16) {
         addHRegUse(u, HRmRead,  insn->variant.dfp_compare.op1_lo);  
         addHRegUse(u, HRmRead,  insn->variant.dfp_compare.op2_lo);  
      }
      break;
   ...
   case S390_INSN_DFP_CONVERT:
      ...
      addHRegUse(u, HRmRead,  insn->variant.dfp_convert.op_hi);  
      if (! hregIsInvalid(insn->variant.dfp_convert.op_lo))
         addHRegUse(u, HRmRead, insn->variant.dfp_convert.op_lo); 
      break;

   case S390_INSN_DFP_REROUND:
      addHRegUse(u, HRmWrite, insn->variant.dfp_reround.dst_hi);
      addHRegUse(u, HRmRead,  insn->variant.dfp_reround.op2);     
      addHRegUse(u, HRmRead,  insn->variant.dfp_reround.op3_hi);  
      if (insn->size == 16) {
         addHRegUse(u, HRmWrite, insn->variant.dfp_reround.dst_lo);
         addHRegUse(u, HRmRead,  insn->variant.dfp_reround.op3_lo); 
      }
      break;
...
s390_emit_load_64imm(UChar *p, UChar reg, ULong val)
{
   ...
   if (ulong_fits_unsigned_32bit(val)) {
      p = s390_emit_LLILH(p, reg, (val >> 16) & 0xFFFF); 
...
s390_emit_LBRw(UChar *p, UChar r1, UChar r2)
{
   ...
   p = s390_emit_SLL(p, r1, R0, 24);          
   return s390_emit_SRA(p, r1, R0, 24);       
}
...
s390_emit_LGBRw(UChar *p, UChar r1, UChar r2)
{
   ...
   p = s390_emit_LR(p, r1, r2);                       
   p = s390_emit_SLLG(p, r1, r1, R0, DISP20(56));     
   return s390_emit_SRAG(p, r1, r1, R0, DISP20(56));  
}
...
s390_emit_LHRw(UChar *p, UChar r1, UChar r2)
{
   ...
   p = s390_emit_LR(p, r1, r2);               
   p = s390_emit_SLL(p, r1, R0, 16);          
   return s390_emit_SRA(p, r1, R0, 16);       
}
...
s390_emit_LGHRw(UChar *p, UChar r1, UChar r2)
{
   ...
   p = s390_emit_LR(p, r1, r2);               
   p = s390_emit_SLLG(p, r1, r1, R0, DISP20(48));     
   return s390_emit_SRAG(p, r1, r1, R0, DISP20(48));  
}
...
s390_emit_LLILFw(UChar *p, UChar r1, UInt i2)
{
   ...
   p = s390_emit_LLILH(p, r1, (i2 >> 16) & 0xFFFF);  
...
s390_insn_divs(UChar size, HReg rem, HReg op1, s390_opnd_RMI op2)
{
   ...
   insn->size = size;
   insn->variant.divs.rem = rem;   
   insn->variant.divs.op1 = op1;   
...
s390_cc_as_string(s390_cc_t cc)
{
   ...
   case S390_CC_H:      return "greater than";     
   case S390_CC_NLE:    return "not low or equal";
   case S390_CC_L:      return "less than";        
   case S390_CC_NHE:    return "not high or equal";
   case S390_CC_LH:     return "low or high";
   case S390_CC_NE:     return "not equal";        
   case S390_CC_E:      return "equal";            
   case S390_CC_NLH:    return "not low or high";
   case S390_CC_HE:     return "greater or equal"; 
   case S390_CC_NL:     return "not low";          
   case S390_CC_LE:     return "less or equal";    
...
s390_insn_as_string(const s390_insn *insn)
{
   ...
   case S390_INSN_ALU:
      ...
      s390_sprintf(buf, "%M %R,%O", op, insn->variant.alu.dst, 
...
{
   p = s390_emit_LGHI(p, reg, 0);  
   p = s390_emit_IPM(p, reg, reg);
   
   return s390_emit_SRL(p, reg, 0, 28); 
}
...
s390_insn_move_emit(UChar *buf, const s390_insn *insn)
{
   ...
   if (dst_class == src_class) {
      ...
      if (dst_class == HRcFlt64 && src_class == HRcInt64) {
         if (insn->size == 4) {
            buf = s390_emit_SLLG(buf, R0, src, 0, DISP20(32)); 
      ...
      if (dst_class == HRcInt64 && src_class == HRcFlt64) {
         if (insn->size == 4) {
            ...
            return s390_emit_SRLG(buf, dst, dst, 0, DISP20(32)); 
         } else {
...
s390_insn_alu_emit(UChar *buf, const s390_insn *insn)
{
   ...
   if (op2.tag == S390_OPND_AMODE) {
      ...
      if (insn->variant.alu.tag == S390_ALU_LSH ||
         ...
         return s390_emit_LGR(buf, b2, R0);  
      }
...
s390_insn_cc2bool_emit(UChar *buf, const s390_insn *insn)
{
   ...
   if (cond == S390_CC_ALWAYS)
      return s390_emit_LGHI(buf, r1, 1);  

   buf = s390_emit_load_cc(buf, r1);                 
   buf = s390_emit_LGHI(buf, R0, cond);              
   buf = s390_emit_SLLG(buf, r1, R0, r1, DISP20(0)); 
   buf = s390_emit_SRLG(buf, r1, r1, 0,  DISP20(3)); 
   buf = s390_emit_NILL(buf, r1, 1);                 
...
s390_insn_cas_emit(UChar *buf, const s390_insn *insn)
{
   ...
   r1 = hregNumber(insn->variant.cas.op1); 
...
{
   UChar r1, r1p1, r3,  b, old_high, old_low, scratch;
   Int d;
   s390_amode *am;
   s390_cdas *cdas = insn->variant.cdas.details;

   r1   = hregNumber(cdas->op1_high); 
   r1p1 = hregNumber(cdas->op1_low);  
...
s390_insn_clz_emit(UChar *buf, const s390_insn *insn)
{
   ...
   p = s390_emit_BRC(p, S390_CC_E, (4 + 4 + 6 + 4 + 4)/ 2);  
   p = s390_emit_AGHI(p, r1, (UShort)-1);         
   p = s390_emit_SRLG(p, R0, R0, R0, DISP20(1));  
   p = s390_emit_LTGR(p, R0, R0);                 
   p = s390_emit_BRC(p, S390_CC_NE,               
                     (UShort)(-(4 + 6 + 4) / 2));
   return p;
...
s390_insn_helper_call_emit(UChar *buf, const s390_insn *insn)
{
   ...
   buf = s390_emit_LFPC(buf, S390_REGNO_STACK_POINTER,          
...
s390_insn_set_fpc_bfprm_emit(UChar *buf, const s390_insn *insn)
{
   ...
   buf = s390_emit_NILL(buf, R0, 0xFFF8); 
   buf = s390_emit_OR(buf, R0, mode);     
   buf = s390_emit_SFPC(buf, R0);         
...
s390_insn_set_fpc_dfprm_emit(UChar *buf, const s390_insn *insn)
{
   ...
   buf = s390_emit_NILL(buf, R0, 0xFF8F); 
   buf = s390_emit_SLL(buf, mode, 0, 4);  
   buf = s390_emit_OR(buf, R0, mode);     
   buf = s390_emit_SFPC(buf, R0);         
                       ...
                       VexEndness endness_host)
{
   ...
   if (s390_host_has_gie) {
      buf = s390_emit_ASI(buf, -1, b, DISP20(d));   
   } else {
      buf = s390_emit_LHI(buf, R0, -1);             
      buf = s390_emit_A(buf, R0, 0, b, d);          
      buf = s390_emit_ST(buf, R0, 0, b, d);         
   }

   
   buf = s390_emit_BRC(buf, S390_CC_HE, (4 + 6 + 2) / 2);  
