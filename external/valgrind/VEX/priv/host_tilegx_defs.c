                               ...
                               HReg srcR )
{
  ...
  i->GXin.Mul.widening = wid; 
  i->GXin.Mul.sz32 = sz32;    
  i->GXin.Mul.dst = dst;
...
void genSpill_TILEGX (  HInstr ** i1,  HInstr ** i2, HReg rreg,
                       Int offsetB )
{
  TILEGXAMode *am;
  vassert(offsetB >= 0);
...
void genReload_TILEGX (  HInstr ** i1,  HInstr ** i2, HReg rreg,
                        Int offsetB )
{
  TILEGXAMode *am;
  vassert(!hregIsVirtual(rreg));
                       ...
                       void*  disp_cp_xassisted )
{
  ...
  case GXin_Alu: {
    ...
    UInt r_srcR = immR ? (-1)  : iregNo(srcR->GXrh.Reg.reg);
  ...
  case GXin_XDirect: {
    ...
    p = mkLoadImm_EXACTLY4(p,  11, (ULong)i->GXin.XDirect.dstGA);

    p = do_load_or_store_machine_word(p, False  ,  11,
                                      i->GXin.XDirect.amPC);
  ...
  case GXin_EvCheck: {
    ...
    p = do_load_or_store_machine_word(p, True  ,  11,
  ...
  case GXin_ProfInc: {
    ...
    p = mkLoadImm_EXACTLY4(p,  11, 0x6555655565556555ULL);
