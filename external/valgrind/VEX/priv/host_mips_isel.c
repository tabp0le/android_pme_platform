...
{
   HReg reg = mkHReg(True,
                     HRcGPR(env->mode64), 0, env->vreg_ctr);
   env->vreg_ctr++;
   return reg;
}
...
{
   HReg reg = mkHReg(True,
                     HRcFPR(env->mode64), 0, env->vreg_ctr);
   env->vreg_ctr++;
   return reg;
}
...
static void doHelperCall(UInt*   stackAdjustAfterCall,
                         RetLoc* retloc,
                         ISelEnv* env,
                         IRExpr* guard,
   ...
   switch (e->tag) {
      ...
      case Iex_Binop: {
         ...
         if (e->Iex.Binop.op == Iop_Max32U) {
            ...
            MIPSRH *argRH = iselWordExpr_RH(env, False  ,
                                           e->Iex.Binop.arg2);
   ...
   case Iex_Unop: {
      ...
      switch (op_unop) {
         ...
         case Iop_32HIto16: {
            HReg r_dst = newVRegI(env);
            HReg r_src = iselWordExpr_R(env, e->Iex.Unop.arg);
            addInstr(env, MIPSInstr_Shft(Mshft_SRL, True ,
         ...
         case Iop_64HIto32: {
            if (env->mode64) {
               HReg r_dst = newVRegI(env);
               HReg r_src = iselWordExpr_R(env, e->Iex.Unop.arg);
               addInstr(env, MIPSInstr_Shft(Mshft_SRA, False ,
   ...
   if (e->tag == Iex_Binop) {
      ...
      switch (op_binop) {
         ...
         case Iop_F32toI64S: {
            ...
            addInstr(env, MIPSInstr_FpLdSt(False  , 8, tmpD,
                                           am_addr));
      ...
      case Iop_ReinterpI64asF64: {
         ...
         if (mode64) {
            ...
            addInstr(env, MIPSInstr_FpGpMove(MFpGpMove_dmtc1, r_dst, fr_src));
             ...
             r_dst = newVRegD(env);
             iselInt64Expr(&Hi, &Lo, env, e->Iex.Unop.arg);
             r_dst = mk_LoadRR32toFPR(env, Hi, Lo);  
         }
         return r_dst;
      }
      case Iop_I32StoF64: {
      ...
      switch (e->Iex.Binop.op) {
         ...
         case Iop_I64StoF64: {
            ...
            if (mode64) {
               ...
               addInstr(env, MIPSInstr_FpLdSt(True , 8, tmp, am_addr));
               ...
               tmp = mk_LoadRR32toFPR(env, Hi, Lo);  
            }
