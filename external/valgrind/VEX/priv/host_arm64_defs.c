...
static void showARM64VecModifyOp(const HChar** nm,
                                 const HChar** ar,
                                 ARM64VecModifyOp op ) {
   switch (op) {
...
static void showARM64VecShiftImmOp(const HChar** nm,
                                   const HChar** ar,
                      ...
                      const void* disp_cp_xassisted )
{
   ...
   switch (i->tag) {
      ...
      case ARM64in_XDirect: {
         ...
         p = imm64_to_ireg(p, 9, i->ARM64in.XDirect.dstGA);
         ...
         p = imm64_to_ireg_EXACTLY4(p, 9, (Addr)disp_cp_chain_me);
         *p++ = 0xD63F0120;
      ...
      case ARM64in_XIndir: {
         ...
         p = imm64_to_ireg(p, 9, (Addr)disp_cp_xindir);
         *p++ = 0xD61F0120; 
      ...
      case ARM64in_XAssisted: {
         ...
         vassert(trcval != 0);
         p = imm64_to_ireg(p, 21, (ULong)trcval);

         
         
         p = imm64_to_ireg(p, 9, (Addr)disp_cp_xassisted);
         *p++ = 0xD61F0120; 
      ...
      case ARM64in_Call: {
         ...
         p = imm64_to_ireg( (UInt*)p, 9, (ULong)i->ARM64in.Call.target );
      ...
      case ARM64in_ProfInc: {
         p = imm64_to_ireg_EXACTLY4(p, 9, 0x6555755585559566ULL);
         *p++ = 0xF9400128;
         *p++ = 0x91000508;
         *p++ = 0xF9000128;
                                   ...
                                   const void* place_to_jump_to )
{
   ...
   vassert(is_imm64_to_ireg_EXACTLY4(
              p, 9, (Addr)disp_cp_chain_me_EXPECTED));
   vassert(p[4] == 0xD63F0120);

   (void)imm64_to_ireg_EXACTLY4(p, 9, (Addr)place_to_jump_to);
   p[4] = 0xD61F0120;
                                     ...
                                     const void* disp_cp_chain_me )
{
   ...
   vassert(0 == (3 & (HWord)p));
   vassert(is_imm64_to_ireg_EXACTLY4(
              p, 9, (Addr)place_to_jump_to_EXPECTED));
   vassert(p[4] == 0xD61F0120);

   (void)imm64_to_ireg_EXACTLY4(p, 9, (Addr)disp_cp_chain_me);
   p[4] = 0xD63F0120;
                                   ...
                                   const ULong* location_of_counter )
{
   ...
   vassert(0 == (3 & (HWord)p));
   vassert(is_imm64_to_ireg_EXACTLY4(p, 9, 0x6555755585559566ULL));
   vassert(p[4] == 0xF9400128);
   vassert(p[5] == 0x91000508);
   vassert(p[6] == 0xF9000128);
   imm64_to_ireg_EXACTLY4(p, 9, (Addr)location_of_counter);
